...
class Graph implements Iterable<Vertex> {
    public int flag=0,wt,time=0; //TO DETERMINE THE TYPE OF GRAPH WHILE READING THE INPUT
    ...
    void addDirectedEdge(int a, int b, int weight) {
        ...
        head.Adj.add(e);
        tail.revAdj.add(e);
        if(wt==weight && time==1 && wt>0 && flag!=2 && flag!=3) flag=1; //BFS
        else if(weight>0 && wt!=weight && flag!=3) flag=2; //Dijkstras
        else if(weight<0) flag=3; //Bellman
        if(time==0) time=1;
        /*tail.degree++;*/ //increment the in-degree of the vertex
    }
